...
export function ProviderLogin() {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'login') {
      ...
      const { error } = await signUp(email, password, name, phone, 'doctor');
      if (error) { setError(error); setLoading(false); return; }
      navigate('/provider/dashboard');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col px-6 pt-16 pb-8">
      ...
      {mode === 'signup' && (
        <>
          <div className="mb-4">
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Dr. Your Name" className="input-field" />
          </div>
          <div className="mb-4">
          ...
          <div className="mb-4">
            <label className="font-semibold text-sm text-ink mb-2 block">Specialty</label>
            <select value={specialty} onChange={(e) => setSpecialty(e.target.value)} className="input-field">
              {['General Physician', 'Cardiologist', 'Dermatologist', 'Neurologist', 'Orthopedics', 'Pediatrician'].map((s) => (
      ...
      <p className="text-center mt-8 text-sm text-ink-secondary">
        ...
        <button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(null); }} className="font-bold text-primary">
